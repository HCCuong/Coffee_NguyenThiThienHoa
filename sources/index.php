...
$about = $cache->get("select name$lang, desc$lang from #_static where type = ? limit 0,1", array('gioi-thieu'), 'fetch', 7200);
...
$slider_gt = $cache->get("select name$lang, photo, link, numb from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('slide-gt'), 'result', 7200);
$slider_dv = $cache->get("select name$lang, photo from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('slide-dv'), 'result', 7200);
$slider_ads = $cache->get("select name$lang, photo, link from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('slide-ads'), 'result', 7200);
...
$videonb = $cache->get("select link_video, id, name$lang,photo from #_photo where type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status)", array('video'), 'result', 7200);
...
$albumlist = $cache->get("select name$lang, slugvi, slugen, id, photo from #_product where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('thu-vien-anh'), 'result', 7200);
$customer = $cache->get("select name$lang, slugvi, slugen, desc$lang, id, photo, content$lang from #_news where type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb,id desc", array('y-kien'), 'result', 7200);
